...
# MAGIC <img width="1px" src="https://ppxrzfxige.execute-api.us-west-2.amazonaws.com/v1/analytics?category=lakehouse&org_id=1444828305810485&notebook=%2F04-Data-Science-ML%2F04.2-AutoML-patient-admission-risk&demo_name=lakehouse-hls-readmission&event=VIEW&path=%2F_dbdemos%2Flakehouse%2Flakehouse-hls-readmission%2F04-Data-Science-ML%2F04.2-AutoML-patient-admission-risk&version=1"> 
# MAGIC
# MAGIC <!-- no change to nb -->
# MAGIC
...
# MAGIC
# MAGIC ---   
# MAGIC For this demo, we'll consider that our model is ready and register it in our Unity Catalog Model Registry as the current "champion" alias for deployment:
...
client.set_registered_model_alias(name=f"{catalog}.{db}.{model_name}", alias="champion", version=model_registered.version)
...
# MAGIC
# MAGIC Open the Unity Catalog to explore [the dbdemos_hls_patient_readmission model](/explore/data/models/mmt_demos/hls_readmission_dbdemoinit/dbdemos_hls_pr) artifact and analyze the parameters used, including traceability to the notebook used for its creation.
...
# MAGIC
# MAGIC - Batch inferences in notebook [04.3a-Batch-Scoring-patient-readmission]($./04.3a-Batch-Scoring-patient-readmission) to start using it for identifying patient at risk and providing cusom care to reduce readmission risk,
# MAGIC - Real time inference with [04.4a-Model-Serving-with-InferenceTable-patient-readmission]($./04.4a-Model-Serving-with-InferenceTable-patient-readmission) to enable realtime capabilities and instantly get insight for a specific patient.
