...
# endpoint link https://e2-demo-field-eng.cloud.databricks.com/ml/endpoints/uci_xgbCpredict_endpoint?o=1444828305810485

# COMMAND ----------

# DBTITLE 1,Adding bulk 'ground-truth' labels for example dataset
## readmission_label (ground truth) 

from pyspark.sql import functions as F 
import datetime

LabelsNpredictTable_List = [
                            'mmt_mlops_demos.monitoring_dev.uci_xgb_classifier_predict_20230901_20231001_inference_20240222',
                            'mmt_mlops_demos.monitoring_dev.uci_xgb_classifier_predict_20231001_20231101_inference_20240222',
                            'mmt_mlops_demos.monitoring_dev.uci_xgb_classifier_predict_20231101_20231201_inference_20240222',
                            'mmt_mlops_demos.monitoring_dev.uci_xgb_classifier_predict_20231201_20240101_inference_20240222'
                            ]

for en, t in enumerate(LabelsNpredictTable_List):
  if en ==0:
    print(en, t)
    LabelsNpredictions_sDF = spark.read.table(t)
    LabelsNpredictions_sDF = LabelsNpredictions_sDF.withColumn('table_name', F.lit(t))
  else:
    print('append', en, t)
    tmpSDF = spark.read.table(t)
    tmpSDF = tmpSDF.withColumn('table_name', F.lit(t))
    LabelsNpredictions_sDF = LabelsNpredictions_sDF.union(tmpSDF)


## save to UC
datetoday = str(datetime.datetime.now().date()).replace('-','')
LabelsNpredictions_sDF.write.mode('overwrite').saveAsTable(f'mmt_mlops_demos.monitoring_dev.uci_xgb_classifier_predict_20230901_20240101_combi_inference_{datetoday}')

# COMMAND ----------

# LabelsNpredictions_sDF.groupby('table_name').count().show(truncate=False)

# COMMAND ----------

# DBTITLE 1,Required parameters >> widgets
from pyspark.sql import functions as F, types as T

"""
Required parameters in order to run this notebook.
"""
ENDPOINT_NAME = "uci_xgbCpredict_endpoint"       # Name of the model serving endpoint
...
GRANULARITIES = ["1 day", "1 month"]                        # Window sizes to analyze data over
...
LABEL_COL = "readmission_label" #None                                 # Name of columns holding labels. Change this if you join your own labels via JOIN_TABLES above.
...
REQUEST_FIELDS =  [T.StructField('encounter_id', T.LongType(), True),
                   T.StructField('discharge_disposition_id', T.LongType(), True),
                   T.StructField('number_diagnoses', T.LongType(), True),
                   T.StructField('number_emergency', T.LongType(), True),
                   T.StructField('number_inpatient', T.LongType(), True),
                   T.StructField('number_outpatient', T.LongType(), True),
                   T.StructField('weight', T.StringType(), True),
                   T.StructField('payer_code', T.StringType(), True),
                   T.StructField('age', T.StringType(), True),
                   T.StructField('admission_type_id', T.LongType(), True),
                   T.StructField('race', T.StringType(), True),
                   T.StructField('num_procedures', T.LongType(), True),
                   T.StructField('num_lab_procedures', T.LongType(), True),
                   T.StructField('num_medications', T.LongType(), True),
                   T.StructField('hospital_stay_1weekORmore', T.StringType(), True),
                   #T.StructField('readmission_label', T.IntegerType(), True)
                   ] 
                   #"label_predict" #None
...
RESPONSE_FIELD = T.StructField('predictions', T.LongType(), True) #None
...
JOIN_TABLES = [
    ...
    # table_name, preserve_cols, join_cols
    ("mmt_mlops_demos.monitoring_dev.uci_xgb_classifier_predict_20230901_20240101_combi_inference_20240222",
     ["readmission_label","encounter_id"],
     ["encounter_id"]
     )
]
...
PROCESSING_WINDOW_DAYS = 500 #365
...
def collect_inference_table_config(endpoint_name: str) -> InferenceTableConfig:
    ...
    print('catalog',catalog, 'schema',schema, 'table_name_prefix',table_name_prefix)
...
if REQUEST_FIELDS is None or RESPONSE_FIELD is None:
    print("REQUEST_FIELDS is None or RESPONSE_FIELD is None")
...
# MAGIC %md 
# MAGIC ##### Showing that if `REQUEST_FIELDS` and `RESPONSE_FIELD` not specified in the above "Required Parameters" section, the code snippet above will try to `infer_request_fields` and `inferred_response_field` from logged MLFlow model signature 
...
# DBTITLE 1,[if needed] removing ENDPOINT checkpoint wrt inference_payload date & timestamp_ms update
...
requests_unpacked = process_requests(
    ...
    request_fields=REQUEST_FIELDS, ## used if available otherwise inferred
    response_field=RESPONSE_FIELD, ## used if available otherwise inferred 
)
...
# MAGIC %md 
# MAGIC Here we are just checking to see if the variables are properly unrolled/unpacked 
...
# This is specific to the data example + corresponding inference -- groundtruth [make a clone for sharing and remove this ]
display(requests_unpacked.groupby('databricks_request_id').count())
...
# %sql
# drop table `mmt_mlops_demos`.`monitoring_dev`.`uci_xgbCpredict_unpacked` 
...
# This is specific to the data example + corresponding inference -- groundtruth [make a clone for sharing and remove this ]
display(spark.read.table("mmt_mlops_demos.monitoring_dev.uci_xgbcpredict_processed").groupby('__db_date').count().sort('__db_date'))
...
## some syntax "bugs"/typos
(inference_table_config.catalog, #[1:-1], 
inference_table_config.schema,) #[1:-1]
# the `` not required... 
...
# spark.sql("CREATE SCHEMA IF NOT EXISTS mmt_mlops_demos.monitoring_dev")

# Use the catalog/schema of the payload table as the output schema.
# output_schema_name = f"{inference_table_config.catalog}.{inference_table_config.schema}"
output_schema_name = f"{inference_table_config.catalog[1:-1]}.{inference_table_config.schema[1:-1]}"
...
#   info = lm.get_monitor(table_name=processed_requests_table_name)
#   time.sleep(10)
...
# dashboard link https://e2-demo-field-eng.cloud.databricks.com/sql/dashboards/7e6c53c3-def3-42bb-90af-1362ebedb040?f_3bb0d421-c997-4fc7-b7e8-4406ae98dc9d=No+slice&f_df18e430-68e0-498b-b4b3-c11f8df0bfcd=No+slice&f_37df9bb3-a02d-495e-83c2-567475237a2a=1+day&f_913ca623-9843-4a62-81e7-2560d9c8c2db=*
