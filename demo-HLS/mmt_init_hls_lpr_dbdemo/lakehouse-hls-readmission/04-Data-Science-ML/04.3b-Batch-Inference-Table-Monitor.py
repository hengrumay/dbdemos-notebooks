...
# DBTITLE 1,run demo setup so we are already in demo UC
...
# MAGIC %md
# MAGIC ### Review Batch Inference Table
# MAGIC
...
# MAGIC %sql
# MAGIC select count(*) from training_dataset

# COMMAND ----------

# DBTITLE 1,check joins
# MAGIC %sql
# MAGIC SELECT count(*)
# MAGIC FROM patient_readmission_prediction prp
# MAGIC JOIN training_dataset td ON prp.patient_id = td.patient_id
# MAGIC -- JOIN training_dataset td ON prp.encounter_id = td.encounter_id
...
# MAGIC %sql
# MAGIC SELECT prp.*, td.30_DAY_READMISSION
# MAGIC FROM patient_readmission_prediction prp
# MAGIC JOIN training_dataset td ON prp.encounter_id = td.encounter_id
# MAGIC LIMIT 10
...
# MAGIC
# MAGIC For simplification purposes, we will create the baseline table from the pre-existing `training_dataset` table
...
# %sql
# -- select count(*) -- 142395
# SELECT training_dataset.*, CAST(patient_readmission_prediction.risk_prediction AS DOUBLE) AS risk_prediction, model_info
#   FROM training_dataset
#   JOIN patient_readmission_prediction
#   ON training_dataset.patient_id = patient_readmission_prediction.patient_id
#   AND training_dataset.encounter_id = patient_readmission_prediction.encounter_id
#   LIMIT 10

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT training_dataset.*, CAST(patient_readmission_prediction.risk_prediction AS DOUBLE) AS risk_prediction, model_info
...
# MAGIC https://docs.databricks.com/en/lakehouse-monitoring/custom-metrics.html 

# COMMAND ----------

# DBTITLE 1,adapt for specific use_case metric
...
# MAGIC ### Create monitor
# MAGIC
# MAGIC As we are monitoring an inference table (including machine learning model predcitions data), we will pick an Inference profile ([AWS](https://docs.databricks.com/en/lakehouse-monitoring/create-monitor-api.html#inferencelog-profile) | [Azure](https://learn.microsoft.com/en-us/azure/databricks/lakehouse-monitoring/create-monitor-api#inferencelog-profile)) for the monitor.
# MAGIC

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from hls_readmission_batch_inference limit 10
...
info = w.quality_monitors.create(
  ...
  inference_log=MonitorInferenceLog(
        ...
        prediction_col="risk_prediction",
        timestamp_col="current_datetime",
        granularities=["1 day"],
        model_id_col="model_info",
...
# MAGIC <!-- https://e2-demo-field-eng.cloud.databricks.com/dashboardsv3/01ef8cbcc4051428b0cf5a151c3d0b20/published?o=1444828305810485 -->
# MAGIC
